...
# --- Grimoire patch: drop bundled Wayland client libs ---
# App forces GDK_BACKEND=x11 (tauri#8541) and never talks to Wayland directly,
# so these only risk ABI clashes with the host's newer libwayland → EGL_BAD_PARAMETER.
for _lib in libwayland-client libwayland-egl libwayland-server libwayland-cursor; do
  rm -fv "$APPDIR"/usr/lib/"$_lib".so*   2>/dev/null || true
  rm -fv "$APPDIR"/usr/lib64/"$_lib".so* 2>/dev/null || true
done
